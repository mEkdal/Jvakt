package Jvakt;
/*
 * 2024-12-29 V.02 Michael Ekdal		Added the "Save log as.." functionality
...
public class consoleLogsView extends JFrame implements TableModelListener, WindowListener {
	...
	public static void main(String[] args) throws IOException {
		...
		System.out.println("consoleLogsView "+ver);
	...
	public consoleLogsView() throws IOException {
		...
		setTitle("Jvakt consoleLogsView "+getVersion()+"."+ver+" --- "+id+" - "+origin+" - "+credat);
		//	        setSize(5000, 5000);
		...
		menuItem.addActionListener(saveLog());
		menu.add(menuItem);
		menuItem = new JMenuItem("Help (F1)");
		menuItem.addActionListener(showHelp());
	...
	private void addKeyBindings() {
		...
		table.getActionMap().put("saveLog", saveLog());
			...
			public void actionPerformed(ActionEvent e)  {
				...
				JOptionPane.showMessageDialog(getContentPane(),
						"F1 : Help \nF3 : Increase font size \nF4 : Decrease font size\nF7 : Show line \n\nESC : Unselect " +
								...
								"\n\n\"File/Save Log as..\" gives the opportunity to save the log to a file.\n" 
								,"Jvakt Help",
	...
	private AbstractAction saveLog()  {
		AbstractAction save = new AbstractAction() {
			static final long serialVersionUID = 49L;
			@Override
			public void actionPerformed(ActionEvent e)  {
				fileChooser.setDialogTitle("Specify file to save log");    
				...
				if (userSelection == JFileChooser.APPROVE_OPTION) {
					...
					System.out.println("Rader "+wD.getRowCount());

					try {
						fis = new FileOutputStream(fileToSave.getAbsolutePath(), false);
					...
					try {
						for (int i=0; i<wD.getRowCount()-1; i++) {
//							System.out.println((String) wD.getValueAt(i, 1));
							logg.write((String) wD.getValueAt(i, 1));
							logg.newLine();				    	
						}
						logg.close();
					}
